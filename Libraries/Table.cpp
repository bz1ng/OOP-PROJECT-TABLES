...
class Table{
    ...
    void Create(std::string name /*std::string path*/, int rows, int columns){ // Creates a new table
        ...
        if (File){
            ...
            cells = new Cell*[rows];
            for (int i = 0; i < rows; i++) cells[i] = new Cell[columns];
    ...
    void SetData( std::string data, int row, int column){ // Sets data to a cell function
        if (this->rows < row || this->columns < column) std::cout << "Error: " << "row" << row + 1 << ", col" << column + 1 << " is out of range" << std::endl; // If the row or column is out of range
        else if(cells[row][column].setData(data) == 0) std::cout << "Error: "<< "row" << row + 1 << ", col" << column + 1 << ", "<< data << " is unknown data type" << std::endl; // If the data is invalid
    }
    ...
    void CalculateFunction(Cell& current){ // Calculates the value of a function in table
        ...
        if (left[0] == 'R') // If the left side is a cell
        {
            ...
            if (ROW > this->rows || COLUMN > this->columns || ROW < 0 || COLUMN < 0) std::cout << "Out of range" << std::endl; // If the cell is out of range
